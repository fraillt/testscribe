...
/// `standalone(...)` accepts several wrapper attributes; `ignore` makes libtest skip the
/// whole tree, which is proven here by a check that would fail the suite if it ever ran.
#[testscribe(standalone(test, ignore))]
fn ignored_standalone_tree_never_runs() {
    then!("this tree is skipped by the harness").run(|| false);
}
