...
Traditionally, it's common to write a single test for the "happy path" and assert only on the final state, then add a few more tests that cover failure cases. When you run these tests, you might see output like:
```text
...
## How `#[testscribe]` wires tests together

Two pieces of "magic" make the tree work. Both are generated by the `#[testscribe]` attribute, so you rarely write them by hand — but knowing they exist explains the syntax you see in every test.

### The `then!` macro is injected, not imported

`#[testscribe]` injects a `then!` macro into the body of every annotated test function. That is why you never `use` it, and why it only works inside `#[testscribe]` functions: besides the value you pass it, `then!` also captures a hidden test-execution-context value (the reporter that records each check into the tree). Outside an annotated function that context does not exist, so `then!` is intentionally unavailable there.

### Every test function generates a PascalCase type

Each test function also generates a type named after it in PascalCase (`one_and_two_is_added` → `OneAndTwoIsAdded`). A child test declares its parent by taking `Given<ParentPascalCaseType>` and receives the parent's returned state through it.

When a node returns a named struct, its *node type* and its *state type* are two different names — the child names the parent and destructures the state it returns:

```rust
#[testscribe]                                    // node:  ItemsAddedToCart
fn items_added_to_cart(/* ... */) -> FilledCart { /* ... */ }   // state: FilledCart

#[testscribe]
fn cart_checked_out(Given(FilledCart { cart_id, .. }): Given<ItemsAddedToCart>) { /* ... */ }
```

Binding parents through a generated type (rather than a string) serves two purposes:

- **Type-system wiring.** The parent relationship is checked by the compiler. The alternative would be naming the function in the attribute, e.g. `#[testscribe(given(parent_function))]` — a stringly-typed link the compiler could not verify.
- **IDE navigation.** Because the link is a real type, "go to definition" and "find references" jump between a parent and its children like any other type usage.
...
Keep the trunk on the happy path: each child continues from the previous step's **success**. Failures, cancellations and refunds branch off it and stay shallow.
...
- [Checkout service](https://github.com/fraillt/testscribe/blob/main/examples/checkout-domain/README.md) - a more realistic end-to-end example.
- [Does your use case fit testscribe?](./does_it_fit.md) - the rough edges: when testscribe fits (linear stories, test matrices) and when it doesn't (concurrency, property tests, benchmarks).
...
- **Cloning external state**: when optimizing execution speed, your `Clone` / `CloneAsync` implementation may need to duplicate external resources. See [Advanced Techniques — State cloning](./advanced_techniques.md) for patterns and trade-offs.
