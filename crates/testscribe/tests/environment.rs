...
// `Env<E>` is written the same way in sync and async tests — the macro inserts the
// anonymous lifetime that async functions require, so you never spell out `Env<'_, E>`.
