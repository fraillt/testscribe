...
/// Every test also generates a PascalCase type (here `OneAndTwoIsAdded`) that child tests bind
/// to via `Given<...>` to access the parent's result — see foundations.md, "How `#[testscribe]`
/// wires tests together", for how that (and the injected `then!` macro) works.
