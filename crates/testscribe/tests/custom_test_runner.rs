...
// A custom runner needs internals the prelude deliberately leaves out (`standalone::*`,
// `CASES`), so this file keeps explicit imports rather than `use testscribe::prelude::*;`.
...
/// For example, running `cargo test --package testscribe --test custom_test_runner -- [slow]`
...
fn main() -> ExitCode {
    ...
    // For example `cargo test --package testscribe --test custom_test_runner -- --include-ignored`
